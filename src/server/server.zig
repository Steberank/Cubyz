			...
			while (y != lastBoxEnd[1]) : (y +%= chunk.chunkSize) {
				...
				while (z != lastBoxEnd[2]) : (z +%= chunk.chunkSize) {
					...
					if (!inXDistance or !inYDistance or !inZDistance) {
						if (@as(i64, z) + chunk.chunkSize <= chunk.worldMinZ) continue;
...
pub fn connectInternal(user: *User) void {
	...
	// A player joining mid game needs the current weather right away; the
	// periodic broadcast is up to two seconds away.
	if (world) |w| main.network.protocols.genericUpdate.sendWeather(user.conn, &w.weather);
