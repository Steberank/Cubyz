...
pub const ServerWorld = struct { // MARK: ServerWorld
	...
	/// Authoritative cloud formations. Broadcast to clients, which never spawn
	...
	pub fn loadWorldConfig(self: *ServerWorld, arena: NeverFailingAllocator, dir: main.files.Dir, worldData: ZonElement) !void { // MARK: loadWorldConfig
		...
		self.weather = .init(self.settings.seed);
		if (worldData.getChildOrNull("weather")) |weatherZon| {
			self.weather.loadFromZon(weatherZon);
		}
	}
	...
	pub fn saveWorldConfig(self: *ServerWorld) !void {
		...
		worldData.put("weather", self.weather.toZon(main.stackAllocator));
	...
	/// Formations only matter where somebody can see them, so they are spawned
	/// around a connected player, falling back to the world spawn.
	fn updateWeather(self: *ServerWorld, deltaTime: f32) void {
		const userList = server.getUserList(main.stackAllocator);
		defer main.stackAllocator.free(userList);

		const anchor: vec.Vec2d = if (userList.len != 0) blk: {
			const pos = userList[0].player().pos;
			break :blk .{pos[0]/main.weather.cloudScale, pos[1]/main.weather.cloudScale};
		} else .{
			@as(f64, @floatFromInt(self.spawn[0]))/main.weather.cloudScale,
			@as(f64, @floatFromInt(self.spawn[1]))/main.weather.cloudScale,
		};

		self.weather.update(anchor, deltaTime);
	}
	...
	pub fn update(self: *ServerWorld) void { // MARK: update()
		...
		if (self.lastUnimportantDataSent.durationTo(newTime).toSeconds() > 2) {
			...
			self.weather.dirty = true;
		}
		if (self.weather.dirty) {
			self.weather.dirty = false;
			const userList = server.getUserList(main.stackAllocator);
			defer main.stackAllocator.free(userList);
			for (userList) |user| {
				main.network.protocols.genericUpdate.sendTime(user.conn, self);
				main.network.protocols.genericUpdate.sendWeather(user.conn, &self.weather);
			}
		}
		self.tick();
		// TODO: Entities
