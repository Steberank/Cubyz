...
pub fn main(args: std.process.Init.Minimal) void { // MARK: main()
	...
	// `initThreadLocals` runs before the IO layer exists, so its clock-based
	// seed was always the same. Draw a real one now that random is available.
	{
		var bytes: [@sizeOf(u64)]u8 = undefined;
		io.random(&bytes);
		seed = std.mem.readInt(u64, &bytes, .little);
	}
	...
	// The server decides what weather a formation brings, so it needs the cloud
	// types just as much as the renderer does.
	weather.CloudType.initRegistry();
	defer weather.CloudType.deinitRegistry();
