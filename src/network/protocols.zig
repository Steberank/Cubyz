...
pub const genericUpdate = struct { // MARK: genericUpdate
	...
	fn clientReceive(conn: *Connection, reader: *utils.BinaryReader) !void {
		switch (try reader.readEnum(UpdateType)) {
			...
			.weather => {
				var regions: main.List(main.weather.Region) = .empty;
				defer regions.deinit(main.stackAllocator);

				const count = try reader.readInt(u8);
				for (0..count) |_| {
					// Formations of a cloud type this client does not have are
					// skipped rather than shown as the wrong weather.
					if (try main.weather.Region.read(reader)) |region| regions.append(main.stackAllocator, region);
				}
				main.renderer.clouds.regions.receiveSnapshot(regions.items);
			},
			.particles => {
	...
	fn serverReceive(conn: *Connection, reader: *utils.BinaryReader) !void {
		switch (try reader.readEnum(UpdateType)) {
			.gamemode, .teleport, .time, .biome, .particles, .clear, .weather => return error.InvalidSide,
			.worldEditPos => {
				const typ = try reader.readEnum(WorldEditPosition);
				const pos: ?Vec3i = switch (typ) {
					.selectedPos1, .selectedPos2 => try reader.readVec(Vec3i),
	...
	pub fn sendWeather(conn: *Connection, worldWeather: *const main.server.Weather) void {
		var writer = utils.BinaryWriter.init(main.stackAllocator);
		defer writer.deinit();

		writer.writeEnum(UpdateType, .weather);
		worldWeather.write(&writer);
