...
pub const ParticleManager = struct { // MARK: ParticleManager
	...
	pub fn register(assetsFolder: []const u8, id: []const u8, zon: ZonElement) void {
		...
		var particleType = readTextureDataAndParticleType(assetsFolder, textureId);
		// The size otherwise follows the texture resolution, which makes small
		// particles impossible to draw in any detail.
		if (zon.get(f32, "size")) |size| particleType.size = size;
	...
	fn readTextureDataAndParticleType(assetsFolder: []const u8, textureId: []const u8) ParticleType {
		...
		// One texel per 1/16 of a block, unless the type overrides it.
