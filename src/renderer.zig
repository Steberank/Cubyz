...
pub fn worldDepthTexture() c_uint {
	return worldFrameBuffer.depthTexture;
}
...
pub fn renderWorld(world: *World, ambientLight: Vec3f, skyColor: Vec3f, playerPos: Vec3d, deltaTime: f64) void { // MARK: renderWorld()
	...
	// Kicked off early so the cloud compute dispatches overlap with rasterizing
	// the terrain.
	gpu_performance_measuring.startQuery(.cloud_meshing);
	clouds.prepare(&frustum, playerPos, deltaTime);
	gpu_performance_measuring.stopQuery();

	rain.update(playerPos, deltaTime);
	...
	// Clouds go in before the transparent terrain so that water and glass blend
	// over them, and before the deferred pass so the fog and bloom reach them.
	gpu_performance_measuring.startQuery(.cloud_rendering);
	clouds.render(&frustum, playerPos, ambientLight, worldFrameBuffer.depthTexture, lastWidth, lastHeight);
	worldFrameBuffer.bind();
	c.glViewport(0, 0, lastWidth, lastHeight);
	gpu_performance_measuring.stopQuery();
	...
	// Rain is drawn on top of the finished frame so that the world fog does not
	// wash it out. See `rain.render`.
	gpu_performance_measuring.startQuery(.rain_rendering);
	rain.render(playerPos, ambientLight);
	gpu_performance_measuring.stopQuery();

	c.glBindFramebuffer(c.GL_FRAMEBUFFER, 0);

	gpu_performance_measuring.startQuery(.hud);
